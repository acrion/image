...
#include "acrion/image/drawing.hpp"
...
// The shape primitives. They exist so that "acrion image tools" can expose drawing as
// messages - until now a plugin could set one pixel at a time through Lua, which is both
// unusable and, at one Lua call per pixel, slow enough to make the point moot.
//
// Every case below asserts what was drawn *and* what was not: a shape that is one pixel too
// large is a defect that only shows up when someone draws two of them next to each other.

namespace
{
    using namespace acrion::image::drawing;

    /// A black canvas and a white pen, so that "was this pixel drawn" is a single comparison.
    BitmapData<uint8_t> Canvas(const int width, const int height)
    {
        BitmapData<uint8_t> data(width, height, 1);
        data.Set(Color<uint8_t>(0));
        return data;
    }

    const Color<uint8_t> white{255};

    /// How many pixels of the canvas are not black.
    size_t DrawnCount(const BitmapData<uint8_t>& data)
    {
        size_t count = 0;
        for (int y = 0; y < data.Height(); ++y)
            for (int x = 0; x < data.Width(); ++x)
                if (data.GetGray(x, y) != 0) ++count;
        return count;
    }
}

TEST(DrawingTest, ALineConnectsItsTwoEndpoints)
{
    auto canvas = Canvas(20, 20);
    Line(canvas, 2, 3, 17, 3, white);

    EXPECT_EQ(canvas.GetGray(2, 3), 255);
    EXPECT_EQ(canvas.GetGray(17, 3), 255);
    EXPECT_EQ(canvas.GetGray(10, 3), 255);
    EXPECT_EQ(canvas.GetGray(1, 3), 0);  // not one pixel too far
    EXPECT_EQ(canvas.GetGray(18, 3), 0);
    EXPECT_EQ(DrawnCount(canvas), 16u);  // inclusive of both ends: 17 - 2 + 1
}

TEST(DrawingTest, ALineIsClippedRatherThanRejected)
{
    // Half off the canvas has to draw its visible half. A widget that scrolls would
    // otherwise blink out entirely as soon as one end left the view.
    auto canvas = Canvas(20, 20);
    Line(canvas, -5, 10, 5, 10, white);

    EXPECT_EQ(canvas.GetGray(0, 10), 255);
    EXPECT_EQ(canvas.GetGray(5, 10), 255);
    EXPECT_EQ(DrawnCount(canvas), 6u);
}

TEST(DrawingTest, TheClipRectangleKeepsDrawingInsideIt)
{
    auto canvas = Canvas(20, 20);
    Line(canvas, 0, 5, 19, 5, white, 1, Rect{4, 0, 6, 20});

    EXPECT_EQ(canvas.GetGray(3, 5), 0);
    EXPECT_EQ(canvas.GetGray(4, 5), 255);
    EXPECT_EQ(canvas.GetGray(9, 5), 255);
    EXPECT_EQ(canvas.GetGray(10, 5), 0);
    EXPECT_EQ(DrawnCount(canvas), 6u);
}

TEST(DrawingTest, AThickLineIsThick)
{
    auto canvas = Canvas(21, 21);
    Line(canvas, 10, 2, 10, 18, white, 3);

    // Three pixels across, centred on the line.
    EXPECT_EQ(canvas.GetGray(9, 10), 255);
    EXPECT_EQ(canvas.GetGray(10, 10), 255);
    EXPECT_EQ(canvas.GetGray(11, 10), 255);
    EXPECT_EQ(canvas.GetGray(8, 10), 0);
    EXPECT_EQ(canvas.GetGray(12, 10), 0);
}

TEST(DrawingTest, ARectangleCoversExactlyItsWidthAndHeight)
{
    auto canvas = Canvas(20, 20);
    Rectangle(canvas, 4, 5, 6, 3, white, true);

    EXPECT_EQ(canvas.GetGray(4, 5), 255);
    EXPECT_EQ(canvas.GetGray(9, 7), 255);   // x + w - 1, y + h - 1
    EXPECT_EQ(canvas.GetGray(10, 7), 0);    // and not one further
    EXPECT_EQ(canvas.GetGray(9, 8), 0);
    EXPECT_EQ(DrawnCount(canvas), 18u);     // 6 * 3
}

TEST(DrawingTest, AnOutlinedRectangleIsHollow)
{
    auto canvas = Canvas(20, 20);
    Rectangle(canvas, 4, 5, 6, 4, white, false);

    EXPECT_EQ(canvas.GetGray(4, 5), 255);
    EXPECT_EQ(canvas.GetGray(5, 6), 0);     // the inside stays empty
    EXPECT_EQ(DrawnCount(canvas), 2u * 6 + 2 * 4 - 4); // both rows, both columns, corners once
}

TEST(DrawingTest, ADegenerateRectangleDrawsNothing)
{
    auto canvas = Canvas(20, 20);
    Rectangle(canvas, 4, 5, 0, 3, white, true);
    Rectangle(canvas, 4, 5, 6, -1, white, true);

    EXPECT_EQ(DrawnCount(canvas), 0u);
}

TEST(DrawingTest, AnEllipseIsSymmetricAboutItsCentre)
{
    // The property a naive parametric sampling breaks, and the reason for the midpoint
    // algorithm: a lopsided circle looks almost right and is almost impossible to spot.
    auto canvas = Canvas(41, 41);
    Ellipse(canvas, 20, 20, 15, 9, white);
    ...
    for (int y = 0; y < 41; ++y)
    {
        for (int x = 0; x < 41; ++x)
        {
            ...
            EXPECT_EQ(canvas.GetGray(x, y), canvas.GetGray(40 - x, y));
            EXPECT_EQ(canvas.GetGray(x, y), canvas.GetGray(x, 40 - y));
        }
    }
}

TEST(DrawingTest, AnEllipseTouchesItsRadiiAndNotBeyond)
{
    auto canvas = Canvas(41, 41);
    Ellipse(canvas, 20, 20, 15, 9, white);

    EXPECT_EQ(canvas.GetGray(35, 20), 255); // cx + rx
    EXPECT_EQ(canvas.GetGray(5, 20), 255);  // cx - rx
    EXPECT_EQ(canvas.GetGray(20, 29), 255); // cy + ry
    EXPECT_EQ(canvas.GetGray(20, 11), 255); // cy - ry
    EXPECT_EQ(canvas.GetGray(36, 20), 0);
    EXPECT_EQ(canvas.GetGray(20, 30), 0);
}

TEST(DrawingTest, AFilledEllipseIsSolidAndStaysInsideItsBounds)
{
    auto canvas = Canvas(41, 41);
    Ellipse(canvas, 20, 20, 15, 9, white, true);

    EXPECT_EQ(canvas.GetGray(20, 20), 255); // the centre
    EXPECT_EQ(canvas.GetGray(30, 20), 255);
    EXPECT_EQ(canvas.GetGray(36, 20), 0);   // still nothing outside rx
    EXPECT_EQ(canvas.GetGray(20, 30), 0);

    // Area, within the discretisation of a 15 by 9 ellipse: pi * rx * ry = 424.
    const size_t drawn = DrawnCount(canvas);
    EXPECT_GT(drawn, 380u);
    EXPECT_LT(drawn, 470u);
}

TEST(DrawingTest, AFilledPolygonFillsItsInteriorOnly)
{
    // A triangle whose area is known: base 10, height 10, so 50 pixels give or take the
    // boundary. The even-odd rule is easy to get wrong at a shared vertex, which shows up as
    // a stray line across the shape at the height of that vertex.
    auto canvas = Canvas(20, 20);
    FilledPolygon(canvas, {{2, 2}, {12, 2}, {2, 12}}, white);

    EXPECT_EQ(canvas.GetGray(3, 3), 255);   // inside
    EXPECT_EQ(canvas.GetGray(11, 11), 0);   // outside the hypotenuse
    EXPECT_EQ(canvas.GetGray(13, 2), 0);    // outside the base
...
TEST(DrawingTest, APolygonWithFewerThanThreePointsDrawsNothing)
{
    auto canvas = Canvas(20, 20);
    FilledPolygon(canvas, {{2, 2}, {12, 2}}, white);

    EXPECT_EQ(DrawnCount(canvas), 0u);
}

TEST(DrawingTest, FloodFillStopsAtAnEdgeItCannotCross)
{
    // A box drawn first, then filled from inside: the fill must stay in, and the count says
    // so exactly rather than approximately.
    auto canvas = Canvas(20, 20);
    Rectangle(canvas, 4, 4, 10, 10, white, false);

    const Color<uint8_t> gray{128};
    const size_t         filled = FloodFill(canvas, 5, 5, gray);

    EXPECT_EQ(filled, 64u); // the 8 by 8 interior
    EXPECT_EQ(canvas.GetGray(5, 5), 128);
    EXPECT_EQ(canvas.GetGray(4, 4), 255); // the wall is untouched
    EXPECT_EQ(canvas.GetGray(3, 3), 0);   // and nothing leaked outside
}

TEST(DrawingTest, FloodFillWithTheColourAlreadyThereTerminates)
{
    // The way a flood fill hangs: the pixels it writes still match the seed, so they are
    // visited for ever. Nothing here asserts a value - the test is that it returns at all.
    auto canvas = Canvas(20, 20);

    const size_t filled = FloodFill(canvas, 5, 5, Color<uint8_t>(0));

    EXPECT_EQ(filled, 0u);
}

TEST(DrawingTest, FloodFillToleranceIncludesNearbyBrightnesses)
{
    auto canvas = Canvas(10, 10);
    canvas.Plot(5, 5, Color<uint8_t>(3)); // almost, but not quite, the background

    EXPECT_EQ(FloodFill(canvas, 0, 0, Color<uint8_t>(200), 0.0), 99u);  // all but that pixel

    auto other = Canvas(10, 10);
    other.Plot(5, 5, Color<uint8_t>(3));
    EXPECT_EQ(FloodFill(other, 0, 0, Color<uint8_t>(200), 5.0), 100u);  // within tolerance
}

TEST(DrawingTest, DrawingWorksForEveryPixelType)
{
    // The primitives are templates; a plugin message dispatches all five depths into them.
    BitmapData<uint16_t> wide(10, 10, 1);
    wide.Set(Color<uint16_t>(0));
    Rectangle(wide, 1, 1, 3, 3, Color<uint16_t>(40000), true);
    EXPECT_EQ(wide.GetGray(2, 2), 40000);

    BitmapData<double> floating(10, 10, 1);
    floating.Set(Color<double>(0.0));
    Ellipse(floating, 5, 5, 3, 3, Color<double>(0.75), true);
    EXPECT_DOUBLE_EQ(floating.GetGray(5, 5), 0.75);

    BitmapData<uint8_t> colour(10, 10, 3);
    colour.Set(Color<uint8_t>(0));
    Line(colour, 0, 0, 9, 0, Color<uint8_t>(200, 30, 30));
    EXPECT_EQ(colour.GetRed(4, 0), 200);
    EXPECT_EQ(colour.GetGreen(4, 0), 30);
}
